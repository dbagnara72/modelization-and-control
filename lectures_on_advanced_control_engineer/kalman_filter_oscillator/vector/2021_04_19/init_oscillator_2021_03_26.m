...
[mag1,phase1,wout1] = bode(plant,options);
Mag1 = 20*log10(mag1(:)); Phase1 = phase1(:);
wout1 = wout1./(2*pi);

[mag2,phase2,wout2] = bode(HPF,options);
Mag2 = 20*log10(mag2(:)); Phase2 = phase2(:);
wout2 = wout2./(2*pi);

figure;
subplot 211; semilogx(wout1,Mag1,'LineWidth',3,'Color',[0 0 0]);
hold on
subplot 211; semilogx(wout2,Mag2,'LineWidth',3,'Color',[0.6 0.6 0.6]); grid on;
hold off
title('$Plant(s)$ and $HPF(s)=1-Plant(s)$ - Bode Diagram','Interpreter','latex');
ylabel('Magnitude','Interpreter','latex');
xlabel('Frequency/Hz','Interpreter','latex');
set(gca,'xlim',[wout1(1) wout1(end)]);
subplot 212; semilogx(wout1,Phase1,'LineWidth',3,'Color',[0 0 0]);
hold on
subplot 212; semilogx(wout2,Phase2,'LineWidth',3,'Color',[0.6 0.6 0.6]); grid on;
hold off
% title('Plant (1-Plant) Bode Diagram','Interpreter','latex');
