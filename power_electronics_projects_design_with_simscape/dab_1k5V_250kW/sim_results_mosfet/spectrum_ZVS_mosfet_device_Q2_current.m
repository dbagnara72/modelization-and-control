...
N2=Nc;
segnale = i_device_Q2_mosfet_zvs_sim(N1:N2);
time = t(N1:N2);
...
title('ZVS Device Q2 Current','Interpreter','latex','FontSize',fontsize_plotting);
...
xlim([f1 f2]);
grid;
title('ZVS Device Q2 Current Spectrum','Interpreter','latex','FontSize',fontsize_plotting);
xlabel('$f/Hz$','Interpreter','latex','FontSize', fontsize_plotting);
ylabel('$i/A$','Interpreter','latex','FontSize', fontsize_plotting);
print('ZVS_device_Q2_current_spectrum','-depsc');

ZVS_device_Q2_current_rms = sqrt(mean(segnale.^2))
