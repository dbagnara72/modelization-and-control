...
N2=Nc;
segnale = mosfet_power_losses_inv_sim(N1:N2);
time = t(N1:N2);
...
title('INV Mosfet Power Loss','Interpreter','latex','FontSize',fontsize_plotting);
...
segnale = mosfet_JH_temp_inv_sim(N1:N2);

subplot 212
plot(time,segnale,'-','LineWidth',tratto2,'Color',colore1);
title('INV Mosfet Junction-Heatsink Temperature','Interpreter','latex','FontSize',fontsize_plotting);
xlabel('$t/s$','Interpreter','latex','FontSize', fontsize_plotting);
ylabel('$T/K$','Interpreter','latex','FontSize', fontsize_plotting);
legend('$T_{JH}^{Q1}$','Location','best',...
    'Interpreter','latex','FontSize',fontsize_plotting);
set(gca,'xlim',[t1c t2c]);
print('inv_mosfet_power_loss','-depsc');
