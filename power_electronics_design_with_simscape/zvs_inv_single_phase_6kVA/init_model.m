...
Cs = 720e-6;
Ls = 1.2e-6;
p_tranf = v1_trafo^2/(2*pi*fPWM_ZVS*Ls)*pi/4
RLs = 0.5e-3;
...
% carico uscita
Rload = 6.25; % valore da usare per carico inverter
...
%% double integrator observer motor
A = [0 1; 0 0];
Aso = eye(2) + A*ts_inv;
Cso = [1 0];
...
delta = 0.02;
res = 10^(22/20) * s/(s^2 + 4*delta*pi*frequency_grid*s + (2*pi*frequency_grid)^2);
