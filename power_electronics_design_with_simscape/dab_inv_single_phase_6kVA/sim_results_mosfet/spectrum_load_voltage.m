...
N2=Nc;
segnale = load_voltage_sim(N1:N2);
time = t(N1:N2);
...
title('Load Voltage','Interpreter','latex','FontSize',fontsize_plotting);
...
xlim([f1 f2]);
grid;
title('Load Voltage Spectrum','Interpreter','latex','FontSize',fontsize_plotting);
xlabel('$f/Hz$','Interpreter','latex','FontSize', fontsize_plotting);
ylabel('$u/V$','Interpreter','latex','FontSize', fontsize_plotting);
print('load_voltage_spectrum','-depsc');

u_load_rms = sqrt(mean(segnale.^2))
